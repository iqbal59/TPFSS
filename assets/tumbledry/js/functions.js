...
function sendFile(file, sender) {
  if (typeof snImageUpload !== 'undefined') {
    ...
    data.append('tumbledry_csrf', csrfToken);
...
function deleteFile(file) {
  if (typeof snDeleteUpload !== 'undefined') {
    $.ajax(
      {
        data: { file: file, tumbledry_csrf: csrfToken },
        type: 'POST',
        url: snDeleteUpload,
        cache: false
      });
 ...
 *
 * In the CI configuration, the "csrf_token_name" must be set as "tumbledry_csrf".
 *
 * @global string csrfToken
 * @param  object $form
 * @return void
...
function formAjaxRequest($form) {
  ...
  if (enctype === 'multipart/form-data') {
    ...
    if (isCsrfTokenExists() === true && addCsrf) {
      formData.append('tumbledry_csrf', csrfToken);
    }
  }
  else {
    if (isCsrfTokenExists() === true && addCsrf) {
...
function getRecord(id, source, $element) {
  ...
  var dataToSend = { id: id, tumbledry_csrf: csrfToken };
