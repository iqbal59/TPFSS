...
class Accounts_model extends CI_Model
{
    ...
    {
        $sql = "select *, (opening_balance+msales+rsales+debit-receipt-credit) as openbalance from (select stores.id, stores.store_crm_code, opening_balance, stores.store_name, firm_name, store_address, ifnull(msales, 0) as msales, ifnull(rsales, 0) as rsales, ifnull(receipt, 0) as receipt, ifnull(debit, 0) as debit, ifnull(credit, 0) as credit , stores.bharatpay_id, paytm_mid1, paytm_mid2, paytm_mid3, gst_type from stores left join (SELECT store_crm_code, sum(amount) as msales FROM `material_invoices` where invoice_date < '$date'  group by store_crm_code) as mtable on (stores.store_crm_code=mtable.store_crm_code) left join (select sum(net_amount) as rsales, store_id from invoices where date(invoice_date) < '$date' group by store_id) rtable on (rtable.store_id=stores.id) left join (SELECT store_id, sum(case when voucher_type = 'R' then amount else 0 end) as receipt, sum(case when voucher_type = 'D' then amount else 0 end) as debit, sum(case when voucher_type = 'C' then amount else 0 end) as credit FROM `vouchers` WHERE 1 and date(create_date) < '$date'  group by store_id) as v on (v.store_id=stores.id)) as openbalancetable where id=$id";
    ...
    {
        $sql = "select * from invoices where store_id='" . $store_id . "' and date(invoice_date) >= '" . $from_dt . "' and date(invoice_date) <= '" . $to_dt . "' and invoice_type=0";
    ...
    public function get_royalty_sale_data($order_nos, $store_name)
    {
        ...
        $sql = "select storesales.order_date, storesales.order_no, COALESCE(refundsales.taxable_amount, storesales.taxable_amount) AS taxable_amount, COALESCE(refundsales.net_amount, storesales.net_amount) AS net_amount, COALESCE(refundsales.service_code, storesales.service_code) AS service_code, storesales.taxable_amount as ta, storesales.net_amount as na, storesales.service_code as sc from storesales left join refundsales on (storesales.order_no=refundsales.order_no and storesales.store_name=refundsales.store_name) where 1 $sql_search and storesales.store_name='" . $store_name . "'";
